...
export function Celebrations() {
  ...
  return (
    <div className="overlay" onClick={dismiss}>
      <Fireworks
        bursts={c.kind === "level" ? 6 : 3}
        key={c.kind === "level" ? `l${c.level}` : `b${c.badge.id}${c.tier}`}
      />
        ...
        {c.kind === "level" ? (
          <>
            ...
            {levelTitle(c.level) !== levelTitle(c.level - 1) ? (
              ...
              <p>
                Continua a esplorare e a catturare: ogni livello è una nuova
                sfida!
              </p>
            )}
            <div className="unlock-list">
              {LEVEL_REWARDS.filter(
                (r) => r.level > (c.from ?? c.level - 1) && r.level <= c.level,
              ).map((r, i) => (
                <button
                  key={r.level}
                ...
                >
                  ...
                  <span className="grow" style={{ textAlign: "left" }}>
                    <span className="muted" style={{ fontSize: 13 }}>
                      <GameIcon name="sparkles" size={13} /> Livello {r.level} ·{" "}
                      {KIND_NAMES[r.kind]}
                    </span>
